...
class VPNSocks5Proxy:
    def __init__(self, host: str = None, port: int = 9898, username: str = None, password: str = None, max_workers: int = 1024):
    ...
    async def run(self):
        ...
        print(f"[*] Async Engine   : Enabled ({self._engine_name})\n")
    ...
    def start(self) -> None:
        self._engine_name = _setup_event_loop()
        asyncio.run(self.run())


def main():
    parser = argparse.ArgumentParser(description="High-Performance Async SOCKS5 Proxy")
    ...
    proxy = VPNSocks5Proxy(host=args.host, port=args.port, username=args.user, password=args.password)
